...
class LoggerManager:
	def __init__(self,log_base: Path, run_id: str = None, mode: str = "offline"):
		...
		#Final path : logs/ONLINE_YYY-MM-DD_HH-MM-SS[_run-XXX]
		self.log_dir = log_base / "logs" / f"{session_label}{run_suffix}"
		self.log_dir.mkdir(parents=True, exist_ok=True)
		
		self.event_log_path = self.log_dir / "event_log.txt"
		logging.basicConfig(level=logging.INFO, filename=self.event_log_path, filemode='w', format='%(asctime)s [%(levelname)s] %(message)s',)
		
		#Also output to terminal
		console_handler = logging.StreamHandler()
		console_handler.setLevel(logging.INFO)
		console_formatter = logging.Formatter('[%(levelname)s')
		...
		with open(self.decoder_csv_path, mode='w',newline='') as f :
			writer = csv.writer(f)
			writer.writerow(["Trial","Timestamp","P(Target)","P(Nontarget)","True Label","Predicted Label","Target Threshold","Non Target Threshold","Phase"])
			
		self.trial_summary_path = self.log_dir / "trial_summary.csv"
		with open(self.trial_summary_path, mode='w', newline='') as f :
				writer = csv.writer(f)
				writer.writerow([
					"Trial","True Label", "Predicted Label", "Accuracy Threshold", "Confidence", "Num Predictions"
				])
	...
	def log_decoder_output(self, trial, timestamp, prob_target, prob_nontarget, true_label, predicted_label, target_threshold, nontarget_threshold, phase):

		with open(self.decoder_csv_path, mode='a', newline='') as f :
			writer = csv.writer(f)
			writer.writerow([
				trial, timestamp, prob_target, prob_nontarget, true_label, predicted_label, target_threshold, nontarget_threshold, phase
			])

	def log_trial_summary(self, trial_number, true_label, predicted_label, accuracy_threshold, confidence, num_preds):

		with open(self.trial_summary_path, mode='a', newline='') as f:
			writer = csv.writer(f)
			writer.writerow([
				trial_number, true_label, predicted_label, accuracy_threshold, confidence, num_preds
			])
	...
	@classmethod
	def auto_detect_subject(cls, subject: str, base_path: Path, mode: str = "offline"):
		...
		try:
			...
			if size2 > size1 :
				session_dir = latest_xdf.parents[1]
				run_id = next((p for p in latest_xdf.stem.split('_') if p.startswitch("run-")), None)
				logger = cls(log_base=session_dir,run_id=run_id, mode=mode)
				logger.log_base = session_dir
