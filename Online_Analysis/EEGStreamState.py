...
class EEGStreamState:
    def __init__(self, inlet: StreamInlet, config, mode="Target", logger=None):
        self.inlet = inlet
        self.config = config
        self.logger = logger
        self.mode = mode
    ...
    def update(self):
        try:
            ...
            if not self.first_chunk_processed:
                ...
                valid_channel_names, valid_raw, valid_indices = (
                    get_valid_channel_mask_and_metadata(
                ...
                if self.mode == "motor":
                    motor_raw = select_channels(
                        valid_raw, keep_channels=self.config.MOTOR_CHANNEL_NAMES
                    )
                    self.subset_indices = [
                        self.channel_names.index(ch) for ch in motor_raw.ch_names
                    ]
                    self.channel_names = motor_raw.ch_names
                    self.final_indices = [
                        self.valid_channel_indices[i] for i in self.subset_indices
                    ]
                elif self.mode == "errp":
                    errp_raw = select_channels(
                        valid_raw, keep_channels=self.config.ERRP_CHANNEL_NAMES
                    )
                    self.subset_indices = [
                        self.channel_names.index(ch) for ch in errp_raw.ch_names
                    ]
            ...
            print(f"⚠️ Failed to update EEG stream: {e}")
            traceback.print_exc()
            raise
    ...
    def compute_baseline(self, duration_sec=1.0):
        ...
        self.baseline_mean = buffer_array.mean(
            axis=0, keepdims=True
        ).T  # shape: (n_channels, 1)
    ...
    def get_baseline_corrected_window(self, window_size_samples):
        ...
        window = np.array(self.filtered_buffer)[
            -window_size_samples:
        ].T  # shape: (n_channels, samples)
